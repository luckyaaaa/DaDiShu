...
public class GameSouthPanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {//这个没s
        ...
        for (int i = 0; i < controller.playerA.frontView.size(); i++) {
            ...
            if (controller.playerA.isDiZhu &&(controller.playerA.frontView.get(i).frontView==controller.surplusCards.get(0).frontView||controller.playerA.frontView.get(i).frontView==controller.surplusCards.get(1).frontView||controller.playerA.frontView.get(i).frontView==controller.surplusCards.get(2).frontView)) {
                g.drawImage(bgImg, 330 + 12 * i, 0, 70, 101, null);//第五参数观察者
            }else {
                g.drawImage(bgImg, 330 + 12 * i, 20, 70, 101, null);//第五参数观察者
            }
        }
